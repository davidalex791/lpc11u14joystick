...
//	uint8_t joystick_status = Joystick_GetStatus();
	uint64_t keys = KeyboardScan();

	CLEAR_HID_MOUSE_REPORT(&g_mouse.report[0]);

	g_mouse.report[0] = Remap(GetADC(0));
	g_mouse.report[1] = Remap(GetADC(1));
	g_mouse.report[2] = Remap(GetADC(2));
	g_mouse.report[3] = Remap(GetADC(3));
	g_mouse.report[4] = Remap(GetADC(4));
	g_mouse.report[5] = Remap(GetADC(5));
	g_mouse.report[6] = Remap(GetADC(6));
	g_mouse.report[7] = Remap(GetADC(7));

	g_mouse.report[8] = (uint8_t)(keys);
	g_mouse.report[9] = (uint8_t)(keys>>8);
	g_mouse.report[10] = (uint8_t)(keys>>16);
	g_mouse.report[11] = (uint8_t)(keys>>24);
	g_mouse.report[12] = (uint8_t)(keys>>32);
}
